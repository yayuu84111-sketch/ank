...
async function createAheButtonAndClick() {
    ...
    try {
        ...
        for (let tokenIndex = 0; tokenIndex < tokens.length; tokenIndex++) {
            ...
            progressText.textContent = `トークン ${tokenIndex + 1}/${tokens.length}: ボタン作成 + ${clickCount}クリック中 (${clicksPerChannel}回/チャンネル/周)...`;
        ...
        resultText.innerHTML = `
            ...
            <p class="text-gray-400">チャンネル数: ${channelIds.length}</p>
            <p class="text-gray-400">トークン数: ${tokens.length}</p>
            <p class="text-gray-400">総クリック回数: ${clickCount}</p>
            <p class="text-gray-400">チャンネル毎クリック: ${clicksPerChannel}</p>
