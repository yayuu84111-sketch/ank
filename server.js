...
app.post('/api/get-text-channels', async (req, res) => {
    try {
        const { token, guildId, skipFilter } = req.body;
        ...
        for (const channel of allChannels) {
            ...
            if (!hasAppCommands || !hasExternalApps) {
        ...
        console.log(`[get-text-channels] Skipped: ${skippedChannels.noView.length} (no view), ${skippedChannels.noSendMessages.length} (no send), ${skippedChannels.noAppCommands.length} (no app commands)`);
...
app.post('/api/check-channel-permissions', async (req, res) => {
    try {
        ...
        for (const channel of allChannels) {
            ...
            const hasAppCommands = (permissions & USE_APPLICATION_COMMANDS) !== 0n;
            const hasExternalApps = (permissions & USE_EXTERNAL_APPS) !== 0n;
            const canUseAppCommands = hasAppCommands && hasExternalApps;
            ...
            if (!canView) {
                ...
                status = 'no_send';
                reason = 'メッセージ送信権限なし';
            } else if (!canUseAppCommands) {
                status = 'no_app_commands';
                reason = '外部アプリコマンド使用不可';
...
app.post('/api/send-dm', async (req, res) => {
    try {
        ...
        const defaultMessage = `# [あんこちゃんですあんこ鯖では主にピクセルガンの無償代行や荒らしを行っていますスマホ勢荒らし知識がなくても自作あんこアプリで初日から最強荒らし勢になろうピクセルガンも初日で最強になろう](<https://nemtudo.me/e/aTPA>)
# 🤍🩷💚🖤❤️🩵🩶🧡💙💛💜🤎❣️💗💝💓❤️‍🩹❤️‍🔥💞💘💖💕
# 😉あんこちゃんだよ！
# 🩵あんこ鯖では主にピクセルガンの無償代行
# 🧡ワイワイ雑談‼️
...
`;
        ...
        const sendMessage = async (token, channelId) => {
            ...
            while (retries < maxRetries) {
                try {
                    let content = message || defaultMessage;
        ...
        console.log(`[send-dm] DONE: ${results.success}/${results.total} in ${elapsed}ms (${msgsPerSec} msg/s, ${results.rateLimitHits} rate-limits)`);
        ...
        if (guildId) {
            ...
            if (!isAdmin) {
                try {
                    ...
                    channels = filteredChannels;
                    console.log(`[INSTANT-PARALLEL] Filtered: ${inputChannels.length} -> ${channels.length} channels (skipped: ${skippedChannels.noView.length} no view, ${skippedChannels.noAppCommands.length} no app commands)`);
...
app.post('/api/ahe-instant-parallel-button', async (req, res) => {
    try {
            ...
            if (!isAdmin) {
                try {
                    ...
                    for (const channelId of inputChannels) {
                        ...
                        filteredChannels.push(channelId);
                        console.log(`[AHE-SEQUENTIAL] Channel ${channelId}: @everyone=${hasMentionEveryone}`);
                    }
        ...
        const buildOptionsForChannel = (channelId) => {
            ...
            if (message) opts.push({ type: 3, name: 'text', value: message });
            if (userIds && userIds.length > 0) opts.push({ type: 3, name: 'user_ids', value: userIds.join(',') });
        ...
        while (clicksSent < totalClicks) {
            ...
            for (let channelIndex = 0; channelIndex < channelCount && clicksSent < totalClicks; channelIndex++) {
                ...
                try {
                    ...
                    if (e.response?.status === 429) {
                        const retryAfter = (e.response?.data?.retry_after || 2) * 1000;
                        console.log(`[AHE-SEQUENTIAL] Rate limited, waiting ${retryAfter}ms...`);
                        await delay(retryAfter);
                    }
                }
            }
        ...
        console.log(`[AHE-SEQUENTIAL] Phase 2 complete: Fired ${clicksSent}/${totalClicks} clicks`);
